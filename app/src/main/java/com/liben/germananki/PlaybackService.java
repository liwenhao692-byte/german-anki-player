...
public class PlaybackService extends Service {
    ...
    private String spokenWord(String input) { String clean = cleanGerman(input); if (clean.length() == 0) return ""; List<String> words = wordsFrom(clean); if (words.size() >= 2) { String first = words.get(0).toLowerCase(); if (first.equals("der") || first.equals("die") || first.equals("das")) return clean; } if (words.size() == 1) { String article = nounArticles.get(words.get(0).toLowerCase()); if (article != null) return article + " " + words.get(0); } return clean; }
    private String spellBase(String input) { List<String> words = wordsFrom(input); if (words.size() >= 2) { String first = words.get(0).toLowerCase(); if (first.equals("der") || first.equals("die") || first.equals("das")) return words.get(1); } return cleanGerman(input); }
