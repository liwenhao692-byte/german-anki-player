(function(){
  ...
  function cleanWord(s){ return String(s||'').replace(/\(.*?\)/g,'').replace(/^(der|die|das|ein|eine|einen|einem|einer)\s+/i,'').trim(); }
  function spell(s){
    var t = cleanWord(s).replace(/[^A-Za-zÄÖÜäöüß0-9]/g,'');
    var out=[];
    for(var i=0;i<t.length;i++){ out.push(t[i].toLowerCase()==='ß'?'Eszett':t[i].toLowerCase()); }
    return out.join(', ');
  }
    ...
    try{
      ...
      x.onreadystatechange=function(){
        ...
        for(var i=0;i<lines.length;i++){
          if(!lines[i]) continue;
          var p=lines[i].split('\t');
          map[p[0]]={word:p[4]||'', cn:p[6]||'', ex:p[7]||'', excn:p[8]||''};
        }
      };
      x.send();
    }catch(e){}
  ...
  function patchedAudio(url){
    ...
    if(m){
      var r=map[m[1]]||{};
      var type=m[2], text='', lang='de';
      if(type==='word'){ text=r.word; lang='de'; }
      if(type==='spell'){ text=spell(r.word); lang='de'; }
      if(type==='cn'){ text=r.cn; lang='zh-CN'; }
      if(type==='ex'){ text=r.ex; lang='de'; }
      if(type==='excn'){ text=r.excn; lang='zh-CN'; }
